...
public class BlobbyBranchDrawer extends BranchDrawerBase {
		...
		if(branch.activeP) {		
			if(shouldDrawP(branch)) {
				...
				for(int i=0; i<points.size();i++){	
					float tx=branch.length*i/18+p.random(-0.2f,0.2f);
					float ty=branch.length*i/18+p.random(-0.2f,0.2f);
					float mx,my;
					if (i>0){
					...
					drawPoint(growingEndPoint.x, growingEndPoint.y,1);
					else drawPoint(points.get(i).x, points.get(i).y,0);
				}
	...
	public CPoint2 drawLastLine(Vector<CPoint2> points, Branch branch){
		...
		lastPPoint.t = (lastPPoint.t-beforePPoint.t)*howLong*howLong+beforePPoint.t;
		CPoint2 lastCPoint = new PPoint2(lastPPoint.r, lastPPoint.t).toCPoint2();
		if (i>0){
	...
	public void drawPoint(float x, float y, int k){
		p.strokeWeight(0.8f);
		if(k!=0)p.fill(255, 255, 0, 255);
		else p.fill(255, 0, 0, 255);
